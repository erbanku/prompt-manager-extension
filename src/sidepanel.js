...
function renderPrompts(prompts) {
    if (prompts.length === 0) {
        ...
        if (searchTerm === "") {
            promptsList.innerHTML =
                '<p class="empty-state">No prompts saved yet. Click "+" to add one!</p>';
        } else {
            promptsList.innerHTML =
                '<p class="empty-state">No prompts found matching your search.</p>';
        }
    ...
    promptsList.innerHTML = prompts
        .map((prompt, index) => {
            // All prompts show 3 lines preview by default (collapsed)
            const isExpanded = index < 3;
            const textClass = isExpanded ? "" : "collapsed";
    ...
    <div class="prompt-item" data-id="${prompt.id}">
      <div class="prompt-header">
        <button class="btn-expand" data-id="${prompt.id}">
          <svg class="chevron-icon ${chevronClass}" width="14" height="14" viewBox="0 0 12 12" fill="none">
